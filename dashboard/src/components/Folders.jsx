			...
			<table className='table-auto w-full'>
				...
				<tbody>
					{folders.map((folder, index) => (
						<tr key={index} onClick={() => onFolderClick(folder)}>
...
// 					{folders.map((folder, index) => (
// 						<tr key={index}>
// 							<td className="w-5/5 p-3 gap-3 flex flex-row ">
// 								<input type="checkbox" className="ml-2" />
// 								<a href={`/folder/${folder.Nombre}`} className="text-black flex flex-row">
// 									<Folder color="#B369F5" />
// 									<p className='hover:text-purple-800 pl-2'>{folder.Nombre}</p>
// 								</a>
// 							</td>
// 							<td className="p-3 w-1/10 text-right">{folder.Archivos.length}</td>
// 							<td className="p-3 w-1/10 text-right">{folder.Tamaño}</td>
